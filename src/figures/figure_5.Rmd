...
ad_lrt <- output_filt %>%
  ...
  filter(target %in% targets_ad$gene) %>%
  unique()
...
meds_inhib_6m <- inhibitory_neurons_6m %>%
  filter(gene %in% meds_in_6m)

meds_inhib_12m <- inhibitory_neurons_12m %>%
  filter(gene %in% meds_in_12m)

meds_excit_6m <- excitatory_neurons_6m %>%
  filter(gene %in% meds_ex_6m)

meds_excit_12m <- excitatory_neurons_12m %>%
  filter(gene %in% meds_ex_12m)

# combine into 1 df
meds_all <- rbind(meds_inhib_6m, meds_inhib_12m, meds_excit_6m, meds_excit_12m)
```
